...
*select* profile_id [features] [-f, --force] [-q, --quiet] [-b] [--backup=NAME]::
    Activate desired profile. The _profile_id_ can be a profile name
    (e.g. _sssd_, _winbind_) or _@system-default_ to select the distribution's
    default configuration. See profile description with *show* command,
    ...
    *--quiet, -q*:::
        ...
        profile requirements or backup location. Errors are still being printed.
...
*list*::
    List available profiles. The _@system-default_ option will be shown
    if the system default configuration file exists.
...
*check*::
    ...
    * *Current authselect configuration is valid.* -- configuration was
      generated by *authselect*.
    * *System was not configured with authselect.* -- the system is not
      using *authselect*.
    * *No configuration detected.* -- no PAM or nsswitch configuration
      relevant to *authselect*.
      (there might be other pam stacks in /etc/pam.d)

    If the configuration is not valid, the command prints found errors.

    You can refer to RETURN CODES section for specific error codes returned by the command.
...
*@system-default configuration*::
    Distributions can provide a system default configuration by creating
    {AUTHSELECT_DEFAULT_CONFIG_PATH}. This file defines the profile and
    features that will be activated when _@system-default_ is selected.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Use *authselect check*. See description of the `check` subcommand
...
*{AUTHSELECT_DCONF_DIR}/{AUTHSELECT_DCONF_FILE}*::
    Changes to dconf database. The main use case of this file is to set
...
*{AUTHSELECT_DCONF_DIR}/locks/{AUTHSELECT_DCONF_FILE}*::
    This file defines locks on values set in dconf database.
