...
*check*::
    ...
    * *Current authselect configuration is valid.* -- configuration was
      generated by *authselect* and was not modified manually.
    * *Current non-authselect configuration is valid.* -- the system was not
      configured with *authselect* and there are no leftovers from a previous
      *authselect* configuration.
    * *No configuration detected.* -- no PAM or nsswitch configuration was
      found.

    If the configuration was generated by *authselect* but modified manually,
    the command prints an error and exits with code 3.
...
* 0: Success.
* 1: Generic error.
* 2: Profile or configuration was not found, or a valid non-authselect configuration is in use.
